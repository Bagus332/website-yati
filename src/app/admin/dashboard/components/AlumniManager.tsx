...
export default function AlumniManager() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (error) {
      setMessage({
        type: "error",
        text: `Gagal ${editingAlumni ? "memperbarui" : "menambah"} alumni: ${
          error.message
        }`,
      });
    } else {
      setMessage({
        type: "success",
        text: `Alumni berhasil ${
          editingAlumni ? "diperbarui" : "ditambahkan"
        }.`,
      });
      resetForm();
      fetchAlumni();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus data alumni ini?")) {
      ...
      if (error) {
        setMessage({
          type: "error",
          text: "Gagal menghapus alumni: " + error.message,
        });
      } else {
          ...
          <div className="space-y-4">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setBatchEnd(e.target.value);
                if (!isNaN(year)) {
                  setBatchStart((year - 3).toString());
                }
              }}
              className="w-full p-4 bg-white/80 border border-blue-100 rounded-xl focus:ring-2 focus:ring-blue-400 transition-all"
              required
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Lulus {alumni.graduation_year} • Angkatan{" "}
                      {alumni.batch_year}
                    </p>
                  </div>
